...
class InnerProductLayer_v2(nn.Module):
    ...
    def forward(self, feature_emb):
        if self._output_type in ["sum", "bi_vector"]: # FM
        ...
        elif self._output_type == "dot_vector": #FwFM
            inner_product_matrix = torch.bmm(feature_emb, feature_emb.transpose(1, 2)) # [B, num_feat, num_feat]
            flat_upper_triange = torch.masked_select(inner_product_matrix, self.upper_triange_mask)  # [N*(N-1)/2] select upper_triange elements
