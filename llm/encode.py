...
parser.add_argument('--prompt_path', type=str, default=f"./zCTR/llm/llm_emb/{dataset}/field.txt")
parser.add_argument('--llm_emb_path', type=str, default=f"./zCTR/llm/llm_emb/{dataset}/{obj}_{version}_eos.pt") # instruct
parser.add_argument('--llm_emb_path2', type=str, default=f"./zCTR/llm/llm_emb/{dataset}/{obj}_{version}_mean.pt") # instruct_mean   
...
with accelerator.split_between_processes(all_promt_list) as promt_list:
    ...
    for i, batch_input in tqdm(enumerate(batch(promt_list, batch_size)), total=len(promt_list)//batch_size):
        ...
        # print("hidden_states[0]=", hidden_states[0][:, :, :3])
        # print("attentions[0]=", attentions[0][:, 0, :, :])
        # print("hidden_states[1]=", hidden_states[1][:, :, :3])
        # print("attentions[1]=", attentions[1][:, 0, :, :])
        # print("hidden_states[2]=", hidden_states[2][:, :, :3])
        # print("attentions[2]=", attentions[2][:, 0, :, :])
        # exit()

all_predict_embeddings = gather_object(predict_embeddings) # list
# all_predict_embeddings2 = gather_object(predict_embeddings2) # list

# save embeddings
if accelerator.is_main_process:
    all_predict_embeddings = torch.cat(all_predict_embeddings, dim=0) # tensor
    # all_predict_embeddings2 = torch.cat(all_predict_embeddings2, dim=0) # tensor

    print(f"all_predict_embeddings = {all_predict_embeddings.shape}")
    # print(f"all_predict_embeddings2 = {all_predict_embeddings2.shape}")

    # torch.save(all_predict_embeddings.float(), llm_emb_path, pickle_protocol=2, _use_new_zipfile_serialization=False)
    # torch.save(all_predict_embeddings2.float(), llm_emb_path2, pickle_protocol=2, _use_new_zipfile_serialization=False)

    torch.save(all_predict_embeddings.float(), llm_emb_path)
    # torch.save(all_predict_embeddings2.float(), llm_emb_path2)    
