...
def train(
    ...
    # print(f"additional_special_tokens id = {tokenizer(tokenizer.additional_special_tokens[0])['input_ids'][0]}; {tokenizer.additional_special_tokens[0]}")

    # Emb_token_id = tokenizer(tokenizer.additional_special_tokens[0])['input_ids'][0]
...
{data_point["output"]}"""
    ...
    # field_prompt_list = ['User-ID', 'Location', 'Age', 'ISBN', 'Book-Title', 'Book-Author', 'Year-Of-Publication', 'Publisher'   ] 
    # field_prompt_list = ['user_id', 'book_id', 'date_added', 'text_reviews_count', 'series', 'country_code', 'language_code', 'is_ebook', 'average_rating', 'similar_books', 'description', 'format', 'authors', 'publisher', 'num_pages', 'publication_day', 'publication_month', 'edition_information', 'publication_year', 'ratings_count', 'title']
    ...
    # input_field_attention_mask[input_field_id == PAD_ID] = 0


    # use the unique last token to identify the field name
    # input_field_last_col = input_field_id[:, -1] 
    # unique_elements = torch.unique(input_field_last_col, return_counts=False)
    # if unique_elements.size(0) < input_field_last_col.size(0):
    #     print("input_field_last_col=", input_field_last_col)
    #     exit(1)

    # DEBUG
    # tokenizer = AutoTokenizer.from_pretrained('/c23034/wbh/Llama3_Checkpoints')
    ...
    class MyTrainer(transformers.Trainer):
        def compute_loss(self, model, inputs, return_outputs=False):                     
            ...
            # get label field emb
            # labels_last_col = labels_input_ids[:, -2] # [4]
            # _, matched_rows = (input_field_last_col.eq(labels_last_col.view(-1, 1))).nonzero(as_tuple=True)     
            # if len(matched_rows) != labels_input_ids.size(0):
            #     print("labels_last_col:", labels_last_col)
            #     print("input_field_last_col:", input_field_last_col)
            #     print("Please check field_prompt_list!!")
            #     exit()
